...
# %% auto #0
__all__ = ['GitRes', 'callgit', 'get_top', 'Git']
...
def callgit(path, *args, uname=None, pre=None, ok_exit=None):
    ...
    cmd = ['git', '-C', str(fp)] + list(args)
    if pre: cmd = [*pre, *cmd]
    r = subprocess.run(cmd, capture_output=True, text=True)
    if r.returncode not in (0, *listify(ok_exit)): raise CalledProcessError(r.returncode, cmd, r.stdout, r.stderr)
    return GitRes((r.stdout + r.stderr).strip(), r.returncode)
